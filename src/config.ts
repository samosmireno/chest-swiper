...
// Only a 15-streak can mean a flawless 15-card deck, so "perfect" waits for
...
// of the 15-card deck, proven exhaustively in src/leaderboard.test.ts over
// every outcome pattern. The binding case is 11/15 with an 11-streak (1216
// with max speed) vs 12/15 with the misses spread out (streak 3 → 1230).
// Raising STREAK past 11, changing the deck size, or growing the speed
...
// total budget (15 × 0.5 = 7.5, rounded to 8) sits below the 10-pt streak
// step, so speed can only split players the accuracy/streak formula scores
// identically — never reorder.
...
// submissions. 1.2 = the 15-card T1D deck; 1.3 = same deck, score now adds
// the hidden speed bonus with streak reweighted 50 → 10 (so accuracy can
// never be outranked).
export const APP_VERSION = "1.3";
export const SHEETS_WEBHOOK_URL =
  "https://script.google.com/macros/s/AKfycbxPFUcdFAuE3xAz9CWEazUXHEcsSGCIhV3PAUiA2X8RG3Tq_eBK1mFSS8EOWhjObKopLA/exec";
